                                    ...
                                    bool supply_tl2a, gnrc_pktsnip_t *ext_opts)
{
    ...
    if (supply_tl2a) {
        uint8_t* l2src = alloc_shared_block(8 * sizeof(uint8_t));
        size_t l2src_len;
        /* we previously checked if we are the target, so we can take our L2src */
        l2src_len = _get_l2src(iface, l2src, (8 * sizeof(uint8_t)));
                                     ...
                                     ndp_opt_pi_t *pi_opt)
{
    ...
    if (((prefix == NULL) ||
        ...
        ipv6_addr_t* pref_addr = alloc_shared_block(sizeof(ipv6_addr_t));
        ipv6_addr_t tmp = IPV6_ADDR_UNSPECIFIED;
        memcpy(pref_addr, &tmp, sizeof(ipv6_addr_t));
...
static size_t _get_l2src(kernel_pid_t iface, uint8_t *l2src, size_t l2src_maxlen)
{
    bool try_long = false;
    int res;
    uint16_t* l2src_len = alloc_shared_block(sizeof(uint16_t));
